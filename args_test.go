...
func TestArgsValidators(t *testing.T) {
	t.Run("NoArgs", func(t *testing.T) {
		if err := NoArgs.ValidateArgs([]string{}); err != nil {
			t.Errorf("expected nil error, got %v", err)
		}
		if err := NoArgs.ValidateArgs([]string{"foo"}); err == nil {
	...
	t.Run("ExactArgs", func(t *testing.T) {
		val := ExactArgs(2)
		if err := val.ValidateArgs([]string{"a", "b"}); err != nil {
			t.Errorf("expected nil error, got %v", err)
		}
		if err := val.ValidateArgs([]string{"a"}); err == nil {
			t.Errorf("expected error for 1 arg, got nil")
		}
		if err := val.ValidateArgs([]string{"a", "b", "c"}); err == nil {
	...
	t.Run("RangeArgs", func(t *testing.T) {
		val := RangeArgs(1, 3)
		if err := val.ValidateArgs([]string{}); err == nil {
		...
		if err := val.ValidateArgs([]string{"a", "b", "c", "d"}); err == nil {
