...
- They render on the root's help page and no other, count in the root's scope for `Audit` (so a name cannot be declared twice there, while a subcommand may reuse it), and are known to the arity probe — otherwise `myapp --url x somedir` would read the URL's value as a command name.
...
### Added
- **A Command Called With Nothing Shows Its Help.** `mail_cli scan`, which needs a label prefix, answered `accepts 1 arg(s), received 0` — one line that never names the parameter the user is missing, while the page documenting it sat unused. A command invoked with *no arguments at all*, which requires some, now prints its own help to stderr, then the error, and still exits non-zero: a command that printed help instead of running has not succeeded, and a script reading the exit code must not be told otherwise. A *wrong number* of arguments keeps its precise message, because that user tried and a page of help is noise.
- **`ArgsValidator` Reports Its Arity.** It was `func([]string) error`, so `ExactArgs(1)` knew perfectly well that it wanted one argument but sealed that inside a closure; the only way to ask "does this command work with no arguments" was to call it with an empty slice and see. It is an interface now, with `ValidateArgs` and `Arity`. `ArgsFunc` adapts a rule the built-ins cannot express and reports unknown arity, in which case the library asks it once rather than using it as a repeated predicate. The alternative was a second field on `Command` saying the same thing in other words, and two statements of one fact drift — `helpFlagNames` and `bindHelpFlags` were exactly that, and the drift made `Audit` reject examples that ran.
- **`App.Args`**, symmetric with `Command.Args`. It validates the application's own positional arguments and says whether an unrecognised first word is a typo or an argument.

### Changed
- **An Unknown Command Stays Unknown When the Application Defines `Run`.** The check asked "does the enclosing command have a handler", and a handler was taken to own every word after it. That is right for `myapp scan inbox` — `inbox` is `scan`'s argument — but it meant an application defining `App.Run` to handle a bare invocation silently lost typo detection: every misspelling became a positional argument and the program exited 0. Where `Args` did exist it went unread, so a command with a handler and `Args: NoArgs` said `unknown arguments: [bogus]` when it had everything it needed to say `unknown command "bogus". Did you mean …`. The question is now "can the enclosing command take this word as an argument", which the arity makes answerable. Undeclared, an application that has commands is taken to have no positional arguments of its own; one with no commands, or one declaring `Args` that accepts positionals, is unaffected.
- With nothing close enough to suggest, the unknown-command error now names the command that lists the others instead of leaving the user with a rejection and no way forward. That wording was taken from the real application, which had added the sentence itself.
- **The Public API Is Half the Size.** An audit before 1.0 (`review/api-surface-2026-09-18.md`) measured 91 nameable exported entries, of which a complete demo application used 19, 49 were used only by the library's own tests, and 23 of those the library called itself. A capitalised name that only the library invokes is an implementation detail with a promise attached. The surface is now **61**, and the functionality is unchanged: everything removed is either reachable another way or was never reachable from outside at all.
- **`pflag` no longer appears in the public API.** `OptionsValidator` named `*pflag.FlagSet` and `Var` named `pflag.Value`, so writing a custom validator meant importing pflag — and that made clihelp's compatibility promise depend on pflag's, for exactly the users who go furthest with the library. A validator is now handed `clihelp.Flags`, an interface with `Changed` and `Value`: the two questions all four built-in validators ask. `Var` takes `clihelp.Value`, which is pflag.Value's method set under a name clihelp owns, so by Go's structural typing anything that satisfied the old signature still satisfies the new one. `Option.Binder` is unexported, which was the last structural mention.
- **The test harness moved to `clihelp/clihelptest`.** `testing.go` was an ordinary file importing `testing`, so every binary built on this library linked the test framework: 100 dependency packages against 95, and 4,992,085 bytes against 4,968,538 on the example binary. `TestExecute` is `clihelptest.Execute`, `TestExecuteWithStdin` is `clihelptest.ExecuteWithStdin`, and `TestResult` is `clihelptest.Result` — the `net/http/httptest` arrangement. (`flag` remains in the dependency graph; it arrives through pflag's own goflag shim, not through clihelp.)
- **The installer functions are unexported.** Ten names — `InstallCompletion`, `InstallShellIntegration`, `UninstallShellIntegration`, `InstallManPage`, `UninstallManPage`, `CompletionPath`, `IntegrationPath`, `ManPagePath`, `IsCompletionInstalled`, `InstallResult` — all read or write inside the *user's* home directory, and all were already called by the library from `__clihelp` and `CompletionCommand()`. Exporting them offered a second way in that nothing supervised: the rule that the unattended path may only refresh files that already exist lives in `autorefresh.go`, and a consumer calling `InstallShellIntegration` directly got none of it. A documented rule is now one the type system keeps. A packager's real need is untouched — `GenBashCompletion`, `GenZshCompletion`, `GenFishCompletion`, `GenManPage`, `GenShellIntegration`, `GenKeyBindings` and `GenWrapperScript` all write to an `io.Writer` and touch nobody's home.
- **The help system's own renderers are unexported**: `RenderMan`, `RenderFlags` and `RenderHelpTopics` each serve one route through the help command and the library is their only caller; `Explain` exists for the `__explain` protocol call. `RenderGlobalFlags`, whose doc comment read "is an alias for RenderFlags", is deleted. The render API is `RenderGlobal`, `RenderCommand` and the `Render` dispatcher, down from seven entry points with no stated relationship.
- **Duplicates deleted**: `CheckExample` (no caller anywhere, and a duplicate of `ValidateExample`), `ValidateExample` and `ValidateExamples` (unexported — `Audit` validates examples as part of its traversal and is what the README recommends, and `ValidateAllExamples` is the single-error form), `AuditWithOptions` (folded into `Audit`, whose options are variadic so the default is shown rather than documented), `ColorizeExampleLine` (the app-aware form is strictly better), `SplitExampleCommandLine` and `DisplayNameWithArgs` (nothing outside the library used either).
- **`App.AutoInstallCompletion` is deleted**, not carried. It was kept one day earlier as a deprecated alias under a compatibility promise that does not apply before 1.0 — which is the cost this audit exists to stop paying. Use `AutoRefreshIntegration`.
- `AGENTS.md` stated the compatibility rule without its precondition, which is what produced that conservative choice. It now says that before 1.0 a wrong name should be removed rather than aliased, and that the ordinary promise begins at 1.0.

### Documentation
- **The Mechanism, Not the API.** `docs/how-clihelp-decides.md` states the eleven decisions this library makes on the author's behalf, what each reads from `App`, `Command` and `Option`, what it assumes when a field is undeclared, and what is lost by leaving it so — then the invariants a caller can rely on without re-checking, a table of what the library already does so that nobody builds it twice, the escape hatches and their cost, and a complete annotated application. Rules are derivable from a mechanism and a mechanism is not derivable from rules: a real application declared `Args` on all 71 of its commands, exactly as the old rule said, and still wrote 81 lines to get a behaviour that declaration already entitled it to, because the rule named one consequence out of four.
- **Documentation That Names Something Absent Now Fails the Build.** `clihelp.PrintError(err)` appeared in six documents, eight times, since the beginning of the project — it is a method on `App`, so anyone who copied it got a build error and no explanation. `TestDocumentationNamesThingsThatExist` parses the package's own source and fails when the prose names something that is not there, or calls a method as though it were a package function. It found eighteen stale references, three of them functions removed the same day.
- `docs/ai-guidelines.md` is deleted: seven of its eight rules follow from the decision model, and the two things that did not — the pointer-lifetime trap and the canonical application — moved there. `llms.txt` was a 187-line copy of the API that had drifted in five ways; it is now what the llms.txt convention actually specifies, a short index of links, with `go doc` named as the authority that cannot drift.
