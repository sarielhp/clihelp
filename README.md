...
- **Dynamic Autocompletion Callbacks (`Option.Complete`, `Param.Complete`)** — Context-aware completion callbacks for flag values and positional arguments (with `Param.Variadic` support for unbounded trailing arguments), delivering tab suggestions directly into Bash, Zsh, and Fish without custom completion scripts.
